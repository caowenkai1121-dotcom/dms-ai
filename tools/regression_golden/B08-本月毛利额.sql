SELECT COALESCE(SUM(sf.gross_profit),0) AS `毛利额` FROM sales_dw.dws_off_offline_sale_dfn sf WHERE sf.order_date >= DATE_FORMAT(CURDATE(),'%Y-%m-01') AND sf.order_date < DATE_ADD(CURDATE(), INTERVAL 1 DAY) LIMIT 200;
