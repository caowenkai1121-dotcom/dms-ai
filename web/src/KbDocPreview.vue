 ...
 *  excel = Excel 表格：使用服务端解析后的 Markdown，原始排版走下载。 */
...
// Excel（xls/xlsx/xlsm）归 excel：不在浏览器解析不可信工作簿，改用服务端解析内容。
...
/** 预览票据：直链改走 ticket（15 分钟有效），iframe/img 才能绕开 Authorization 头直挂 URL。 */
...
async function loadFile() {
  ...
  try {
    ...
    // Excel 不在浏览器解析用户工作簿；直接复用服务端解析后的 Markdown。
    if (kind === 'excel') {
      if (!markdownRan.value && !markdownLoading.value) void loadMarkdown()
      return
    }
    ...
    <section ref="dialogEl" class="kdp" role="dialog" aria-modal="true" aria-labelledby="kdp-title" tabindex="-1">
      ...
      <div class="kdp-body">
        <section v-show="activeTab === 'file'" class="kdp-pane file" aria-label="原件预览">
          ...
          <div v-else-if="fileKind === 'office' || fileKind === 'excel'" class="kdp-office">
            <!-- 服务端转换版 PDF（探测 206 才进这分支）；转换不可用落解析内容渲染 -->
            <template v-if="fileKind === 'office' && officePdfUrl">
            ...
            <template v-else>
              <p class="kdp-note kdp-office-note">{{ fileKind === 'excel' ? 'Excel 文件按服务端解析后的内容预览；浏览器不再解析用户工作簿，原始排版请下载原件查看。' : 'Office 原件按解析后的内容预览，版式可能与原件有差异；原始排版请下载原件查看。' }}</p>
              <div v-if="markdownLoading" class="kdp-state" role="status">
                <strong>正在解析内容</strong><span>文档解析需要几秒钟。</span>
              </div>
