...
async function load() {
  ...
  error.value = ''
  try {
    const r = await fetch(`/api/skills${authQuery()}`, { headers: authHeaders(false) })
    if (r.status === 401) {
      emit('auth-expired')
    ...
    skills.value = Array.isArray(j.skills) ? j.skills : []
  } catch (e) {
    error.value = `提示词包加载失败（网络）：${e}`
  } finally {
    loading.value = false
  }
}
...
async function toggle(s: Skill) {
  ...
  error.value = ''
  try {
    const r = await fetch(`/api/skills/${s.id}/toggle${authQuery()}`, { method: 'POST', headers: authHeaders(false) })
    if (r.status === 401) {
      emit('auth-expired')
      error.value = '登录已失效，请重新登录'
      return
...
async function removeSkill(s: Skill) {
  ...
  error.value = ''
  try {
    const r = await fetch(`/api/skills/${s.id}${authQuery()}`, { method: 'DELETE', headers: authHeaders(false) })
    if (r.status === 401) {
      emit('auth-expired')
      error.value = '登录已失效，请重新登录'
      return
...
<template>
  <div class="sk-mask" @click.self="emit('close')">
    <section class="sk-dialog" role="dialog" aria-modal="true" aria-labelledby="sk-title">
      <header class="sk-head">
        <div>
          <span class="sk-kicker">提示词包</span>
          <h2 id="sk-title">Skills 管理</h2>
          <p class="sk-sub">注入深度报告规划提示词，admin 可写。启用即注入（最多 5 包、每包前 2000 字），改动后最长约 2 分钟命中旧规划缓存。</p>
        </div>
        <button type="button" class="sk-close" title="关闭" @click="emit('close')">✕</button>
      </header>
      ...
      <form v-if="admin" class="sk-form" @submit.prevent="save">
        ...
        <input v-model="formName" class="sk-input" maxlength="64" placeholder="名称（唯一，最多 64 字）" :disabled="saving" />
        <textarea v-model="formContent" class="sk-textarea" rows="5" placeholder="内容：注入深度报告规划（PLAN）系统提示的文本，最多 20000 字" :disabled="saving"></textarea>
        <div v-if="formError" class="sk-form-err">{{ formError }}</div>
        <div class="sk-form-ops">
          <button type="submit" class="sk-btn primary" :disabled="saving">{{ saving ? '保存中…' : editingId != null ? '保存修改' : '新建' }}</button>
          <button v-if="editingId != null" type="button" class="sk-btn" :disabled="saving" @click="resetForm">取消编辑</button>
          <span class="sk-form-note">新建缺省不启用，保存后在列表里手动开启</span>
        </div>
      </form>
      ...
      <template v-else>
        ...
        <div v-for="s in skills" :key="s.id" class="sk-item" :class="{ off: !s.enabled }">
          <div class="sk-item-hd">
            ...
            <template v-if="admin">
              <button type="button" class="sk-btn" :disabled="busyId === s.id" @click="startEdit(s)">编辑</button>
              <button type="button" class="sk-btn danger" :disabled="busyId === s.id" @click="removeSkill(s)">删除</button>
            </template>
          </div>
          <div class="sk-content" :title="s.content">{{ s.content }}</div>
...
.sk-kicker { display: block; margin-bottom: 5px; color: var(--primary); font-size: 11px; font-weight: 750; }
...
.sk-form-t { font-size: 12px; font-weight: 650; color: var(--text-primary); }
