...
经营日报 21 条 KPI 只有前 5 条进 prompt；`metric_surface_grounded` 抄了第二份指标别名表
（「本月毛利多少」整份合同判废）；深度报告答案形态被三处焊死成固定三段；
few-shot 还在用 trgm 选范例而同一个 join! 里向量已经算好了；
注册表装配器把地名换成码值又不登记证据；`merge_adjacent` 合并块时不合并 `vec_dist`
（昨天刚修好的并列键在合并块上原样复发）；LLM 出口的合同绕开判据是手写黑名单，
`t_master_shop` 不在其中；「设备订单数」不过有效订单状态；量具缺第 8/9 两路诊断。
