...
## AX168 · 先把尺子补上（2026-08-17）

业主把要求收敛成两句：「**1、能准确识别用户的问题，能准确回答用户的问题。
2、智能，能智能地了解用户的意图，能智能地对答案进行整合、进行分析。**」

那就以这两句当验收标准。而今天暴露的三个问题有个共同点：

> **大日期 / 分布 / 明确问数 —— 一个都不在回归题库里。**

于是「回归 87 项全绿」与「用户问的题答不了」可以同时成立。
**尺子量不到的地方，改得再多也不知道有没有变好。** 所以这一轮先补尺子。

### 题库从 86 → 92，每条按「一族」写而不是按「一题」写

| 题 | 代表哪一族 |
|---|---|
| G01 现场口径·大日期商品 | 问句里现场下定义（`X的意思是Y`）。此前整轮落知识库，而 `invalid_date` 一直都在 |
| G02 现场口径·自定义活跃客户 | 同族换一个完全不同的业务对象 —— 证明它是通用能力，不是给大日期开的后门 |
| G03 明确问数不许被资料半抢答 | 用户点了 chip。此前两臂都跑、`hybrid::fuse` 把资料半选成主答案 |
| G04 明确知识库不许被问数抢答 | G03 的反方向，两边都钉，免得修一边把另一边弄反 |
| G05 口语分布问法 | 带「分布/构成/占比」的分组问法。两台生产机同一问句两种答案，尺子里没这条就永远发现不了 |
| G06 仓库名要落到主数据 | 问句给**名称**而事实表只有编码；名称唯一活在 `master_wms.wms_desc` 上 |

### 顺带补上题库的一个能力缺口

`intent_mode` 是**断言**（判返回合同的 mode），不是入参 —— 名字像、作用相反。
于是「用户点了问数 chip」这条路**今天在回归里根本测不到**，题库只会发
`{question, role_code}`。新增 `ask_intent` 入参（`data`/`knowledge`），
缺省不发＝自动分诊，与前端 `intent.value === 'auto'` 时不传 `body.intent` 逐字同口径。
两个请求构造器（CLI 侧与 `ask_http`）都带上，漏一个就是「HTTP 上测不到」。

### 诚实说明

这 6 题在部署之前**必然是红的** —— 那正是它们存在的意义。其中 G01/G06 依赖大模型
生成 SQL，我在本机验不了，只能等它跑在有数据的机器上。

`cargo test --workspace` 2021 通过 / 0 失败；题库自检 92 题字段全合法；部署契约绿。

---

## AX169 · 复杂问句：不是召回不到，是被**哑钉**钉歪了（2026-08-17）

业主：「用户的问题可能很复杂，你需要能准确识别，并且能准确回答。」

上一轮「复杂问句」那条审计线的核验员掉线，等于没结论。这一轮**改用实测**，不再猜 ——
生产上的 `retrieve` 子命令是只读探针（不写 query_log、不触发学习），拿 10 道真复杂问句量召回。

### 实测结论一：召回层不是瓶颈

10 道题（多重限定叠加 / 排除式 / 阈值比较 / 派生指标 / 跨仓对比 / 相对时间窗 /
不完整期 / 财年口径 / 嵌套两段式），**主事实表 9/10 命中**。

### 实测结论二：真问题在 `kw_force` —— 52 条钉子里 16 条（31%）是**哑的**

原始输出里发现「库存周转天数超过90天的SKU」被 `forced: true` 以 score 1.0 强制置顶到
`t_winc_stock_report`。顺着查 `meta.kw_force`（关键词→表，命中即强制置顶，压过一切召回信号），
逮到两件事：

**① 「大日期 / 临期」被巡店模块钉到了 `t_shop_inspection_records`。**
`ops_caliber.rs:524-528` 按运营看板的语境把这两个**通用词**据为己有 ——
而 `meta.kw_force` 的 PK 只有 `(ds_id, keyword)`，是**全局命名空间**，`ON CONFLICT DO UPDATE`
谁后跑谁赢。业主问「京东仓的大日期商品」正是撞在这上面：那两个字在通用业务语境里指的是
**库存效期**（`scm_warehous_manage.invalid_date`），不是巡店文本里命中「大日期」三个字。

**② 但它们其实从来没生效过** —— 这才是更大的那件事。
`recall/schema.rs::forced_tables` 里有一句：

```rust
if !catalog_table(cx.ds, t) { continue; }
```

目标表不在 `warehouse_catalog::ASSETS`（61 条手工白名单）里，这条钉子**直接跳过**。
逐条比对：**52 条里 16 条指向目录之外**。五族逐个实测「召回不到」：

```
湖南省区的市场费用是多少   需要 t_market_total_expense      ❌
各品类的销售额             需要 t_goods_category            ❌
待确认对账单有多少          需要 t_account_bill_header       ❌
巡店记录有多少条           需要 t_shop_inspection_records   ❌
员工有多少人               需要 t_employee                  ❌
```

登记者以为钉上了，实际一次都没命中 —— **registry 里的 16 句谎话**。

### 为什么是「删」不是「把表加进目录」

逐个查过：费用、对账这些域**早已由目录里的 DWS/ADS 表覆盖**
（费用 → `dws_off_sales_cost_dnf` 等四张、对账 → `dws_fin_*_check_dnf` 三张、品类 → `t_goods`），
当年排除 ODS 原表是**有意的口径裁决**。留着钉子只是谎话。
`t_employee` 更不能加 —— 那张表有 `login_pwd` 列，绝不许暴露给自由 SQL。

所以：16 条全部退役（`KW_FORCE_RETIRED` 真删库里的旧行，不是「不再写入」就完事），
巡店模块的「大日期风险」指标保留（那是它自己的定义，不需要靠劫持全局关键词实现）。

### 判据：把「漏项」从静默变成编译期可见

`every_forced_keyword_points_into_the_catalog`：扫两处种子的关键词钉，
**目标表必须在 `ASSETS` 里**；外加「大日期/临期」不许被任何模块钉走；
外加空转跳闸（解析漂了就恒绿）。

反向验证：塞回一条目录外的钉 → 红；把「大日期」钉给任意表 → 红。

**手工名单永远有下一个漏项 —— 但漏了必须有人知道。** 这条判据就是那个「有人知道」。

### 还需要业主拍板的一件事

`库存 → t_winc_stock_report`（营销通经销商库存）这条钉子**是活的**（该表在目录里），
但 `warehouse_catalog.rs` 里 `scm_warehous_manage` 的注释写着
「2026-08-11 用户指定：**库存表用中台的** —— 替代 t_winc_stock_report 成为默认库存源」。
三处口径不一致（目录裁决 / kw_force 钉法 / `stock.rs` 的确定性实现都用 winc）。
没有单方面改 —— 中台表**没有金额列**，改错方向会把「库存金额」类问句弄坏。
问清楚再动。

`cargo test --workspace` **2022 通过 / 0 失败**；部署契约绿。

---

## AX170 · 库存口径按业主裁决收口 + 复杂问句题库（2026-08-17）

### 业主裁决（逐字）

> 「库存是 `scm_warehous_manage` 这张表。`t_winc_stock_report` 只是记录门店的进销存数据，
> 只和门店相关，**没有任何业务逻辑**，`t_winc` 开头的几张表是同步工具从门店直接同步过来的，
> 只用于分析门店的数据，**和 DMS 是独立的**。」

对照代码：`fastpath::stock` 的默认源**已经**是中台（2026-08-11 那次改的），回落营销通时也
已经带「（门店进销存口径）」后缀。真正不一致的只有一处 —— `kw_force` 的
`库存 → t_winc_stock_report`，自由 SQL 那条路被它钉歪。已改指中台。

### 顺带兑现一句拖了很久的注释

`stock.rs` 仓库分组那句写着「仓库码表未镜像进数仓，按码与库位出（**名称接入后再换名**）」。
`master_wms` 今天随业主指路进了目录，那一天到了：

```
改前  GZHYC-777        DHW  12241637.998
改后  皇家小虎长沙黄兴万纬仓xs  DHW  12241637.998
```

`LEFT JOIN` + `COALESCE` 回落编码：实测 31258 行库存里有 12 行没有仓档，
内连会把它们**静默丢掉**（合计对不上）。判据同时钉住「必须出名字」与「必须是左连」。

### 复杂问句题库：按业主点名的五种复杂度建 8 题

> 「可以一个问题很绕，可以一个问题包含多个指标，可以一个问题包含问数和知识库，
> 可以一个问题需要多步拆解，复杂计算等等。」

| 题 | 复杂度 | 数据实测 |
|---|---|---|
| H01 上月冠军省区的本月表现 | **两步拆解**（argmax 后再取数） | 上月冠军「直营」，本月 5253 万 |
| H02 销售额+订单数+毛利率 | **一问多指标且不同源** | 销售额 1.25 亿、毛利率 19.61% |
| H03 市场费用多少 + 报销流程 | **一句跨两条链路**（问数 + 知识库） | — |
| H04 毛利率（问句自带公式） | **现场给公式**，按用户算法算 | — |
| H05 库存量超100万的仓库有几个 | **分组聚合→阈值→计数**（HAVING+外层 COUNT） | 14 个 |
| H06 除了冠军省区以外的合计 | **排除式 + 子查询 argmax** | — |
| H07 京东仓临期但别仓有货 | **同表按仓自连 + 效期 + 名称解析** | — |
| H08 最近三个月每月订单数 | **相对时间窗 + 按月分组 + 当月不完整** | — |

每题都先用只读 `exec-sql` 验过**数据上答得出来**，不建永远过不了的题。
断言口径保守：能确定的钉死（必须落哪张表、不许落知识库），
LLM 具体怎么写 SQL 不钉 —— 钉了就是恒红或恒绿。

题库 86 → 100（G 组 6 题 + H 组 8 题）。

`cargo test --workspace` 2022 通过 / 0 失败；题库 100 题字段全合法；部署契约绿。
