...
/// 两个字段都 `None`（默认）= 不收紧：与全局两档（`dms_agent::MAX_ROWS` / `EXEC_TIMEOUT`）
/// 取 min 恒等，存量行为逐字节不变。配置面（JSON 字段 `max_rows` / `timeout_ms`）在
/// `dms_semantic::registry::datasource::DsPolicyConfig`，这里只留执行形态。
...
/// （`AskCtx.source` 就是这个形状 —— 具名 MySQL 是 `ds_id` 断链的头号成因）。
pub trait SqlSource: Send + Sync {
    ...
    /// 实现侧必须真的存下它（内部可变性）—— 空操作实现等于把管理端配的收紧静默丢地上。
    fn set_ds_policy(&self, policy: DsPolicy);

    /// 取数。`max` = 行上限（超出即截断，不报错）；`t` = 单次查询超时。
    fn fetch<'a>(
        &'a self,
        sql: &'a ScopedSql,
        max: usize,
    ...
    /// 预翻译验证（`EXPLAIN <SQL>`，只解析优化不取数）。
    ///
    /// `Ok(Some(错误))` 仅当**数据库明确判定 SQL 有问题**；超时/连接抖动一律 `Ok(None)`
    /// （不触发改写，见文件头第 2 条）。`Err` 留给「连不上池」这类连 EXPLAIN 都发不出去的情况。
    fn explain<'a>(
        &'a self,
        sql: &'a ScopedSql,
        t: Duration,
...
mod tests {
        ...
        fn fetch<'a>(
            ...
            Box::pin(async move {
                Ok(RowSet {
                    ...
                    rows: vec![vec![1.into(), serde_json::Value::Null]],
    ...
    fn ds_policy_clamp_only_tightens() {
        let (max, t) = (200usize, Duration::from_secs(30)); // 全局两档（dms_agent::MAX_ROWS / EXEC_TIMEOUT）
