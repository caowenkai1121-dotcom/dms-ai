    ...
    pub const fn aliases(self) -> &'static [&'static str] {
        match self {
            Self::SalesAmount => &[
                ...
                // 口语整词（2026-08-11 实测「销售额度按照省份…」残留「度」字被判解析失败）；
                // 整词匹配——「额度」单说（授信额度）不受影响
                "销售额度",
            ],
            Self::SalesQuantity => &[
...
pub const DIMENSIONS: &[Dimension] = &[
    ...
    Dimension::WarZone,
    Dimension::Region,
    // `State` **不在**这里：2026-08-11 裁决「省份 ≡ 省区 ≡ region」管的就是分组口径，
    // 它只给行政省过滤与成员值探针用。`City` 在 —— 那条裁决与市无关。
    Dimension::City,
    Dimension::Month,
];
...
impl Dimension {
    ...
    pub const fn aliases(self) -> &'static [&'static str] {
        match self {
            ...
            // 🔴 「大区」必须在（2026-08-15 生产直打）：war_zone 的取值本身就叫
            // 「东北大区/西北大区/川渝藏大区」，而别名表只有「大战区」——
            // 「各大区本月销售额」于是白拒，同族的「西北大区本月销售额」单点问却能答。
            Self::WarZone => &["销售战区", "大战区", "大区"],
            ...
            Self::State => &[],
            // 🔴 城市**参与**匹配（2026-08-15）：2026-08-11 那条裁决管的是「省份 ≡ 省区」，
            // 与市无关。city 是 dws_off_offline_sale_dfn 的实有列（318 个取值），
