...
async fn seed_warns(pg: &PgPool) -> anyhow::Result<()> {
    const WARNS: &[(&str, &str)] = &[
        ...
        ("t_winc_stock_report", "【⚠️每日快照表(N 天×N 仓)：聚合库存必须 product_stock_date=(SELECT MAX(product_stock_date) FROM t_winc_stock_report) 取最新快照，直接 SUM 会把快照天数累加虚增几十倍。本表是门店/经销商进销存（营销通）口径——默认「库存量」问答用业务中台 ywzt_ods.scm_warehous_manage 的 SUM(in_stock_quantity)+inventory_status='ZP'，勿与本表混加】"),
        ("t_warehouse_manage", "【⚠️管理档案表，不是实时库存(actual_quantity 全 NULL，数值严重失真)。库存问答用 ywzt_ods.scm_warehous_manage（中台 WMS 现行库存）；门店/经销商进销存口径才用 t_winc_stock_report 最新快照】"),
