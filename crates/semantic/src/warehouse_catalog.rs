...
///
/// 🔴 **这不是主力机制**：销售事实那条路由 `ask.rs::dimension_value_hit` 探库确认
/// 成员值（读真数据、新增取值自动跟上），实测「本月直营销售额」「本月零食很忙地配销售额」
/// 都由它接住并给出 verified 结果。本表只服务两个拿不到探针的模板
/// （小程序下单 / ODS 订单，`fastpath::ops`）——业务新增大区时它会陈旧，
/// 但陈旧只会 fail-closed（不认 → 不接），不会答错。
