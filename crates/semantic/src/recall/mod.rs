//! 各召回族的统一入参 `RecallCtx` + 召回族 re-export。
//!
//! 变更原因＝召回的入参形状。搬运前各召回函数在 `server/src/meta.rs` 各带 3~5 个 `&str`/`usize`
//! 形参（`(pg, ds, question)` / `(pg, ds, question, limit)` / `(pg, ds, question, tables, limit)`），
//! 调用点按同一批值逐个重排 —— 收成一个 ctx（D4）。当前调用点：agent/gather.rs、agent/triage.rs、
//! server/corrector.rs、server/direct.rs、server/main.rs。
//!
//! `Copy`：调用点的 `limit` 逐条不同（表召回 6 / 元素 8 / 教训 6），
...
//! 调用侧，semantic 不持 HTTP 客户端）。`None` = embed 服务缺席 → 两条向量路整段降级跳过，
//! 与 gather 侧 `embed_query()` 返 `None` 时的现行降级等价。
...
/// `limit` 的语义随召回族不同（表召回=k，元素/教训=条数上限），且**仅 retrieve/elements/
/// pitfalls 三个召回族读它** —— 指标/维度/术语/码值/值域召回压根不读（gather 波 1 塞的
/// `limit: 6` 对它们是纯占位）。「只有谁读它」的读者清单以 grep 字段名为准。
#[derive(Clone, Copy)]
pub struct RecallCtx<'a> {
    /// 用户问句原文（substring 命中判据吃它，别做归一化——逐字 contains 即行为）
    pub question: &'a str,
    /// 已召回到的物理表名（教训触发词「表名.列名」的表名部分与它比）。只有教训召回读它。
    pub tables: &'a [String],
    /// 条数上限 / 表召回的 k；多数召回族不读此字段（见 struct 注释的读者清单）
    pub limit: usize,
