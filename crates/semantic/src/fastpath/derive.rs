...
pub fn customer_name_fragment(question: &str) -> Option<String> {
    ...
    // 「本月华东区销售额」剥出「华东区」，而 DWS 的 region 值里**没有**华东
    // （只有 广西省区/西北大区/川渝藏大区… 这些形），于是地域路正确地没接。
    // 接住它的是客户模糊探针：`LIKE '%华东区%'` 唯一命中
    // `线下-福建云通供应链有限公司(华东区）`—— 名字**括号里**的地域标注。
    // 于是整句变成「那一家客户本月销售额」，答 0。用户读到的是「华东区本月没销售」。
    //
    // 地理名是**封闭的参考数据**，不是行为模板：一个大区名落到客户名判据上，
    // 唯一正确的结果是不认，让它去走「未确认限定」。
    const MACRO_REGIONS: &[&str] = &["华东", "华南", "华北", "华中", "西南", "西北", "东北", "华西"];
    if MACRO_REGIONS
        .iter()
        .any(|r| name == *r || name == format!("{r}区") || name == format!("{r}地区"))
    {
        return None;
    }
