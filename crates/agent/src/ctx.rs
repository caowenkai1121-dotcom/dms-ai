...
pub struct AskCtx<'a> {
    ...
    pub t0: Instant,
    /// **整轮**起点（`PreparedQuestion::started_at`），含 `prepare_question` 里那次
    /// 意图往返。只给澄清卡用 —— 成员的 `elapsed_ms` 仍用上面的 `t0`（单问语义不变）。
    ///
    /// 🔴 为什么要分两个：意图合同拿不到时，用户**白等 5~10 秒**才拿到一张反问卡，
    /// 而卡上盖的是 `t0` 的章 —— 44ms，只覆盖成员循环那一段。2026-08-17 我照着这个
    /// 数字判定「有东西在调模型之前早退」，开了三路调查才发现模型调了、超时了。
    /// 读日志的人会据此排除掉正确答案，这比没有数字更糟。
    pub turn_t0: Instant,
