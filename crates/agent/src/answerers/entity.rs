...
const TRAILING_INTENT: &[&str] = &[
    // 长尾必须在它们的子串前面（每条每轮只剥一个，「的数据」先于「的销售数据」会把
    // 「…的销售数据」剥成「…的销」）——「X公司的本月数据/销售数据」= 要该实体的经营总览卡
    // （2026-08-12 实测「线下-潍坊程祥商贸有限公司，本月的数据」跌进 need-intent 反问）。
    "的销售数据", "的经营数据", "的数据",
    "的详细资料", "的详细信息", "的基础资料", "的基本信息", "的详细情况", "详细资料",
    "详细信息", "基础资料", "基本信息", "的详情", "的资料", "的信息", "的情况", "详情",
    "资料", "信息", "介绍一下", "介绍", "怎么样", "是什么",
    // 裸尾两形必须在所有「的×」长尾之后：「X公司今年数据」「X公司本月情况」没有「的」
    // （2026-08-12 实测跌反问）；它们先于「的情况」会把「…的情况」剥成「…的」。
    "数据", "情况",
    "吗", "呢",
];
...
const ENTITY_VIEW_TAILS: &[&str] = &[
    ...
    // 带「的」与不带「的」两形都收（生产 LLM 实测会丢「的」：「潍坊程祥本月经营情况」）。
    "的经营情况", "的经营数据", "的经营状况",
    "经营情况", "经营数据", "经营状况",
];
...
mod tests {
    ...
    /// 「实体 + 时间 + 的数据/销售数据」= 要该实体的经营总览卡（带时间窗），不是反问。
    /// 2026-08-12 实测「线下-潍坊程祥商贸有限公司，本月的数据」跌进 need-intent。
    #[test]
    fn month_data_tails_land_on_the_entity_card() {
        assert!(entity_form_hit("线下-潍坊程祥商贸有限公司，本月的数据"));
        assert_eq!(
            bare_name("线下-潍坊程祥商贸有限公司，本月的数据").as_deref(),
            Some("线下-潍坊程祥商贸有限公司")
        );
        // 长尾交叠保护：「的销售数据」不许被「的数据」剥成「…的销」
        assert_eq!(
            bare_name("线下-潍坊程祥商贸有限公司的销售数据").as_deref(),
            Some("线下-潍坊程祥商贸有限公司")
        );
        // 「的数据」尾对非实体句不抢：纯指标问句照旧走自己的路
        assert!(!entity_form_hit("本月销售额的数据"), "没有实体名的指标句不归实体卡");
    }
    ...
    fn entity_parser_only_strips_edge_intent_and_accepts_long_names() {
        ...
        // 模型丢「的」的形态同样要接住（生产实测输出「潍坊程祥本月经营情况」）
        assert_eq!(bare_name("潍坊程祥本月经营情况").as_deref(), Some("潍坊程祥"));
        // 裸尾「数据/情况」：「今年数据」「本月情况」不带「的」也要落实体
        assert_eq!(
            bare_name("线下-潍坊程祥商贸有限公司今年数据").as_deref(),
            Some("线下-潍坊程祥商贸有限公司")
        );
        assert_eq!(bare_name("潍坊程祥本月情况").as_deref(), Some("潍坊程祥"));
        // 反向：带「的」长尾优先于裸尾——「…的情况」不许被剥成「…的」
        assert_eq!(
            bare_name("线下-潍坊程祥商贸有限公司本月的情况").as_deref(),
            Some("线下-潍坊程祥商贸有限公司")
        );
        assert_eq!(
