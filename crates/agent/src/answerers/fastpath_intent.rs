...
pub fn sales_fact_consumed(
    ...
    // 「倒数」同族补丁（2026-08-15）：`detect_top_n`/`rank_direction` 都认它了，
    // 消化词不认就还是残留一个「倒数」被整条拒 —— 与「最低」那颗一个道理。
    consumed.push("倒数".to_string());
...
pub async fn stock_product_filtered(
    ...
    // 🔴 **能力边界 ⇒ 让路；事实缺失 ⇒ 才出卡**（2026-08-17 判官实测收口）。
    //
    // 这一档说的是「本适配器只兑现『商品 + 正品现行库存』，别的维度我做不了」——
    // 那是**我的**局限，不是用户问错了。原来它 `return Some(卡)`，而这个成员排在
    // ROUTER_ORDER 第 3 位，一出卡整轮就收口，第 7 位的自由 SQL **永远轮不到**。
    // 上面注释自己写着「交给上层模型规划或澄清」，而返回 Some 恰恰堵死了「交给模型」。
    //
    // 实测代价：「京东仓的库存量是多少」死在单字「仓」上 —— 而库里 12+ 个仓名
    // 全部以「仓」结尾，这条命中率是 100%。用户拿到「商品限定『京东仓』还不能唯一
    // 匹配到一个库存商品」，而正确答案只要 JOIN 一下主数据就有。
    //
    // 下面探针的三档卡（0 匹配 / 多匹配 / 探针失败）**一个字不动**：那三档说的是
    // 「这段文字指不到唯一商品」，是事实层面的结论，该出卡。
    //
    // 让路的落点是自由 SQL，途中要过语义缓存 —— 缓存的第四关护栏（限定词必须逐字
    // 出现）2026-08-17 同批补上，否则「京东仓的库存量」会回放全仓总量、收据还全绿。
    if UNSUPPORTED.iter().any(|word| fragment.contains(word)) {
        tracing::info!(
            fragment = %fragment,
            "库存商品快路兑现不了这条限定 → 让路给自由 SQL（不是整轮终止）"
        );
        return None;
    }
