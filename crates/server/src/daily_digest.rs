...
//!
//! 调度形态与 A9 向量自愈同一个模子：启动即查 + 10 分钟一轮 + advisory lock
...
const BUSINESS_TZ: chrono::FixedOffset = chrono::FixedOffset::east_opt(8 * 3600).unwrap();
const TODAY_DIGEST_SQL: &str = "SELECT id FROM meta.artifact \
    ...
    tokio::spawn(async move {
        loop {
            match run_round(&st).await {
                Ok(true) => tracing::info!("经营日报：今日份已生成"),
                Ok(false) => {}
                Err(e) => tracing::warn!("经营日报本轮失败（10 分钟后重试）: {e:#}"),
            }
...
async fn run_round(st: &AppState) -> anyhow::Result<bool> {
    ...
    let r: anyhow::Result<bool> = async {
        ...
        st.owned.fixed(crate::admin_api::KV_SET_SQL)
            .bind(KV_DIGEST_DATE).bind(&today).execute().await?;
        Ok(generated)
    }.await;
...
async fn today_digest_exists(st: &AppState) -> anyhow::Result<bool> {
    let row: Option<(i64,)> = st.owned.fixed(TODAY_DIGEST_SQL).fetch_optional().await?;
    Ok(row.is_some())
}
...
async fn generate(st: &AppState) -> anyhow::Result<()> {
    let s = sqls();
    ...
    let kpis_y = one_kpis(st, s.kpis, y, today).await?;
    let kpis_prev_day = one_kpis(st, s.kpis, prev_day, y).await?;
    let kpis_yoy_day = one_kpis(st, s.kpis, yoy_day, yoy_day_end).await?;
    let kpis_mtd = one_kpis(st, s.kpis, month_start, today).await?;
    let kpis_prev_mtd = one_kpis(st, s.kpis, prev_month_start, prev_mtd_end).await?;
    let kpis_yoy_mtd = one_kpis(st, s.kpis, yoy_month_start, yoy_mtd_end).await?;
    let top_region = top(st, s.top_region, y, today).await?;
    let top_customer = top(st, s.top_customer, y, today).await?;
    let trend: Vec<(String, f64)> = st.mysql.raw_dates_all(s.trend, &[t30, today]).await?;
    let orders_y = one_orders(st, y, today).await?;
    let orders_prev_day = one_orders(st, prev_day, y).await?;
    let orders_yoy_day = one_orders(st, yoy_day, yoy_day_end).await?;
    ...
    let insight = if st.insight_enabled {
        ...
        let rows = vec![
            vec![serde_json::Value::from("昨日销售额"), serde_json::Value::from(money_opt(kpis_y.sales_amount))],
            vec![serde_json::Value::from("昨日销售额环比"), serde_json::Value::from(change(kpis_y.sales_amount, kpis_prev_day.sales_amount, false))],
            vec![serde_json::Value::from("昨日销售额同比"), serde_json::Value::from(change(kpis_y.sales_amount, kpis_yoy_day.sales_amount, false))],
            vec![serde_json::Value::from("昨日销量"), serde_json::Value::from(number_opt(kpis_y.sales_quantity))],
            vec![serde_json::Value::from("昨日不含税收入"), serde_json::Value::from(money_opt(kpis_y.revenue_excluding_tax))],
            vec![serde_json::Value::from("昨日不含税成本"), serde_json::Value::from(money_opt(kpis_y.cost_excluding_tax))],
            vec![serde_json::Value::from("昨日毛利额"), serde_json::Value::from(money_opt(kpis_y.gross_profit))],
            vec![serde_json::Value::from("昨日毛利率"), serde_json::Value::from(percent_opt(kpis_y.gross_margin))],
            vec![serde_json::Value::from("昨日毛利率环比"), serde_json::Value::from(change(kpis_y.gross_margin, kpis_prev_day.gross_margin, true))],
            vec![serde_json::Value::from("昨日毛利率同比"), serde_json::Value::from(change(kpis_y.gross_margin, kpis_yoy_day.gross_margin, true))],
            vec![serde_json::Value::from("昨日订单数"), serde_json::Value::from(crate::chart_svg::display_number("订单数", orders_y as f64))],
            vec![serde_json::Value::from("昨日订单数环比"), serde_json::Value::from(change(Some(orders_y as f64), Some(orders_prev_day as f64), false))],
            vec![serde_json::Value::from("昨日订单数同比"), serde_json::Value::from(change(Some(orders_y as f64), Some(orders_yoy_day as f64), false))],
            vec![serde_json::Value::from("当月累计销售额（报告日所在月）"), serde_json::Value::from(money_opt(kpis_mtd.sales_amount))],
            vec![serde_json::Value::from("当月累计销售额环比"), serde_json::Value::from(change(kpis_mtd.sales_amount, kpis_prev_mtd.sales_amount, false))],
            vec![serde_json::Value::from("当月累计销售额同比"), serde_json::Value::from(change(kpis_mtd.sales_amount, kpis_yoy_mtd.sales_amount, false))],
            vec![serde_json::Value::from("当月累计销量"), serde_json::Value::from(number_opt(kpis_mtd.sales_quantity))],
            vec![serde_json::Value::from("当月累计不含税收入"), serde_json::Value::from(money_opt(kpis_mtd.revenue_excluding_tax))],
            vec![serde_json::Value::from("当月累计不含税成本"), serde_json::Value::from(money_opt(kpis_mtd.cost_excluding_tax))],
            vec![serde_json::Value::from("当月累计毛利额"), serde_json::Value::from(money_opt(kpis_mtd.gross_profit))],
            vec![serde_json::Value::from("当月累计毛利率"), serde_json::Value::from(percent_opt(kpis_mtd.gross_margin))],
        ];
...
fn change(current: Option<f64>, baseline: Option<f64>, percentage_points: bool) -> String {
    ...
    if baseline.abs() < f64::EPSILON {
        return if current.abs() < f64::EPSILON { "0.0%" } else { "新增" }.to_string();
    }
...
fn report_md(d: &Digest<'_>) -> String {
    let mut s = format!(
        ...
        od = crate::chart_svg::display_number("订单数", d.orders_y as f64),
        od_mom = change(Some(d.orders_y as f64), Some(d.orders_prev_day as f64), false),
        od_yoy = change(Some(d.orders_y as f64), Some(d.orders_yoy_day as f64), false),
...
mod tests {
    ...
    fn scheduler_is_try_locked_same_conn_and_cas_marked() {
        ...
        let gen = body.find("generate(st).await?").expect("generate 没了");
        ...
        assert!(locked.find("today_digest_exists(st).await?").unwrap()
            < locked.find("generate(st).await?").unwrap());
    }
    ...
    fn prune_keeps_only_todays_newest_digest() {
        ...
        assert!(TODAY_DIGEST_SQL.contains("ORDER BY id DESC LIMIT 1"), "{TODAY_DIGEST_SQL}");
