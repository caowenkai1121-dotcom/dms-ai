...
const PLAN_SYSTEM: &str = "你是资深 BI 分析师。先**深度理解**用户的问题（它到底问什么、\
    ...
    上面那串「省区/战区/客户/商品/月度趋势」**只属于销售经营指标**：退款、售后、库存、\
    费用、账余这些非销售指标，维度只能从目录里**该指标自己带的**列中选；目录没有的维度\
    必须写成数据缺口，不许套用销售那几个维度。给不出第二、第三个可算的维度时宁可只出一个板块\
    ——一份三板块两个红的报表，比一份一板块的报表更没用。";
...
mod tests {
    ...
    fn understanding_is_capped_at_the_prompted_limit() {
        ...
        // 🔴 销售那串维度不许被套到非销售指标上（2026-08-16 业主实测）：
        // 「今年退款额是多少」的深度报告规划出「各战区退款额排行」「各业务类型退款额占比」
        // 两个板块，双双落 need-intent 0 行 —— 售后事实表里根本没有战区。
        // 三板块两个红的报表比一板块的更没用。
        assert!(
            PLAN_SYSTEM.contains("只属于销售经营指标") && PLAN_SYSTEM.contains("宁可只出一个板块"),
            "非销售指标的维度纪律没了：模型会把省区/战区套到退款、库存、费用上"
        );
