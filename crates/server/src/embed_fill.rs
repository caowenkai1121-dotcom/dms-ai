//! 【A9】向量自愈调度：启动即跑一轮 + 每 10 分钟把 `embedding IS NULL` 的行补回。
//!
    ...
    tokio::spawn(async move {
        loop {
            match run_round(&st).await {
                ...
                Err(e) => tracing::warn!("向量自愈本轮失败（10 分钟后重试）: {e:#}"),
            }
...
async fn fill_all(st: &AppState) -> anyhow::Result<u64> {
    ...
    let ds_ids: Vec<String> =
        list_datasources(pg).await?.into_iter().map(|d| d.ds_id).collect();
    for t in MetaVecTarget::ALL {
        if t.ds_scoped() {
            for ds in &ds_ids {
                filled += fill_target(st, t, ds).await?;
            }
        } else {
            filled += fill_target(st, t, "").await?;
        }
    }
...
async fn fill_target(st: &AppState, t: MetaVecTarget, ds: &str) -> anyhow::Result<u64> {
    ...
    let texts: Vec<String> = rows.iter().map(|(_, x)| x.chars().take(1000).collect()).collect();
    ...
    .with_context(|| format!("{t:?} embed 服务缺席，本轮跳过"))?;
    anyhow::ensure!(
        vecs.len() == rows.len(),
        "{t:?} embed 返回条数不符（{} ≠ {}）",
        vecs.len(),
...
async fn fill_kb(st: &AppState) -> anyhow::Result<u64> {
    ...
    if !rows.is_empty() {
        ...
        if let Some(vecs) = st.embed.embed_passages(&texts).await {
            anyhow::ensure!(vecs.len() == rows.len(), "kb embed 返回条数不符");
    ...
    // 块全部补完的 chunked 文档推到 embedded（ingest 正常路径的同款状态迁移）
